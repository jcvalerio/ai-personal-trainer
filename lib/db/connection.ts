...
class DatabaseConnectionFactory {
  ...
  private validateAndCreateConfig(): DatabaseConfig {
    ...
    const NODE_ENV = (process.env.NODE_ENV ||
      'development') as DatabaseEnvironment;
...
function createTypeSafeDatabase(
  connectionFactory: () => NeonQueryFunction<false, false>
  ...
  database.queryRaw = async <T = Record<string, unknown>>(
    ...
    if (values && values.length > 0) {
      ...
      const templateLiteral = Object.assign([sql], {
        raw: [sql],
      }) as TemplateStringsArray;
      return getConnection()(templateLiteral) as Promise<T[]>;
    }
...
export async function checkDbConnection(): Promise<ConnectionHealth> {
  ...
  try {
    ...
    const result = await getDb()
      .query<HealthCheckResult>`SELECT 1 as health_check`;
...
export type DbSingleResult<T = Record<string, unknown>> = Promise<
  T | undefined
>;
