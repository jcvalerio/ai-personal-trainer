    ...
    (set, get) => ({
      ...
      selectAllSessions: (sessionIds) =>
        set((state) => ({
          selection: {
            ...state.selection,
            selectedSessions: new Set(sessionIds),
          },
        })),
      ...
      getFilteredPlans: (plans) => {
        ...
        if (filters.selectedStatus !== 'all') {
          filtered = filtered.filter(
            (plan) => plan.status === filters.selectedStatus
          );
        }
        ...
        if (filters.selectedDifficulty !== 'all') {
          filtered = filtered.filter(
            (plan) => plan.targetFitnessLevel === filters.selectedDifficulty
          );
        }
      ...
      getFilteredSessions: (sessions) => {
        ...
        if (filters.selectedStatus !== 'all') {
          filtered = filtered.filter(
            (session) => session.status === filters.selectedStatus
          );
        }
        ...
        if (filters.timeRange !== 'all') {
          ...
          filtered = filtered.filter(
            (session) => new Date(session.scheduledDate) >= cutoffDate
          );
        }
