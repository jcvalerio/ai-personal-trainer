...
export class WorkoutGenerationProcessor implements JobProcessor {
  ...
  async process(job: Job): Promise<any> {
    ...
    try {
      ...
      const jobResult = await this.aiWorkoutService.getWorkoutGenerationJob(jobId, context)
      ...
      switch (generationJob.jobType) {
        case 'workout_plan':
          result = await this.processWorkoutPlanGeneration(generationJob, context)
          break
        case 'single_session':
          result = await this.processSingleSessionGeneration(generationJob, context)
          break
        case 'exercise_recommendation':
          result = await this.processExerciseRecommendation(generationJob, context)
          break
        default:
   ...
   */
  private async processWorkoutPlanGeneration(generationJob: any, context: any): Promise<any> {
    ...
    console.log('Processing workout plan generation for job:', generationJob.id)
    ...
    const generatedPlan = {
      ...
      estimatedSessionDuration: generationJob.userPreferences.sessionDurationMinutes,
      planData: {
        ...
        phases: [
          {
            ...
            description: 'Build base fitness and establish proper movement patterns',
            durationWeeks: 3,
            sessions: []
          },
          {
        ...
        progressionStrategy: 'Progressive overload with weekly increases in intensity',
        notes: 'Remember to warm up properly and listen to your body'
      },
      weeklySchedule: this.generateMockWeeklySchedule(generationJob.userPreferences.trainingDaysPerWeek),
      progressionRules: {
    ...
    const createResult = await this.workoutService.createWorkoutPlan(generatedPlan, context)
   ...
   */
  private async processSingleSessionGeneration(generationJob: any, context: any): Promise<any> {
    console.log('Processing single session generation for job:', generationJob.id)
    ...
    const createResult = await this.workoutService.createWorkoutSession(generatedSession, context)
    
    if (!createResult.success) {
      throw new Error(`Failed to create workout session: ${createResult.error}`)
    }
   ...
   */
  private async processExerciseRecommendation(generationJob: any, context: any): Promise<any> {
    console.log('Processing exercise recommendation for job:', generationJob.id)
  ...
  private generateMockWeeklySchedule(trainingDays: number): any {
    ...
    for (let week = 1; week <= 8; week++) {
      const weekSchedule = days.map((day, index) => {
        const isTrainingDay = index < trainingDays || (trainingDays > 5 && index === 6)
