...
export class JobManager {
  ...
  private constructor() {
    this.jobQueue = new JobQueueService({
      ...
      cleanupInterval: parseInt(process.env.JOB_QUEUE_CLEANUP_INTERVAL || '3600000'), // 1 hour
  ...
  initialize(): void {
    ...
    this.jobQueue.registerProcessor('workout_generation', new WorkoutGenerationProcessor())
  ...
  async scheduleRecurringJob(
    ...
    return await this.jobQueue.addJob(type, { ...data, recurring: true, cron: cronExpression })
  }
   ...
   * Get jobs by status
   */
  async getJobsByStatus(status: string, limit?: number, offset?: number): Promise<any> {
