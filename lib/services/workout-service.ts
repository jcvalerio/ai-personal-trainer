...
export class WorkoutService extends BaseService {
  ...
  async createWorkoutPlan(
    ...
    try {
      ...
      this.validateRequiredFields(data, ['name', 'durationWeeks', 'sessionsPerWeek', 'fitnessGoals'])
  ...
  async getWorkoutPlans(
    ...
    try {
      ...
      const { clause: whereClause, values } = this.buildWorkoutPlanFilters(filters, context)
  ...
  async getWorkoutPlan(
    ...
    try {
      ...
      const hasAccess = await this.checkResourceAccess(plan.userId, context, true)
      if (!hasAccess && !plan.isPublic) {
        return this.createErrorResult('Access denied', 'UNAUTHORIZED')
      }

      await this.logEvent('plan_accessed', 'Workout plan accessed', context, { planId })
  ...
  async updateWorkoutPlan(
    ...
    try {
      ...
      const updateFields = [
        'name', 'description', 'durationWeeks', 'sessionsPerWeek', 'fitnessGoals',
        'targetFitnessLevel', 'estimatedSessionDuration', 'planData', 'weeklySchedule',
        'progressionRules', 'status', 'isTemplate', 'templateCategory', 'isPublic'
      ]

      updateFields.forEach(field => {
        const dbField = this.camelToSnakeCase(field)
        if (sanitizedData[field] !== undefined) {
          if (['fitnessGoals', 'planData', 'weeklySchedule', 'progressionRules'].includes(field)) {
      ...
      if (updates.length === 0) {
        return this.createSuccessResult(existingPlan.data, 'No changes to update')
      }
      ...
      return this.createSuccessResult(updatedPlan, 'Workout plan updated successfully')
    } catch (error) {
  ...
  async deleteWorkoutPlan(
    ...
    try {
      ...
      await this.logEvent('plan_deleted', 'Workout plan deleted', context, { planId })

      return this.createSuccessResult(true, 'Workout plan deleted successfully')
    } catch (error) {
  ...
  async getWorkoutSessions(
    ...
    try {
      ...
      const { clause: whereClause, values } = this.buildWorkoutSessionFilters(filters, context)
      const offset = ((pagination?.page || 1) - 1) * (pagination?.limit || 10)
      const limit = pagination?.limit || 10
  ...
  async getWorkoutSession(
    ...
    try {
      ...
      // Check access permissions
      const hasAccess = await this.checkResourceAccess(session.userId, context, true)
      if (!hasAccess) {
        return this.createErrorResult('Access denied', 'UNAUTHORIZED')
      }

      await this.logEvent('session_accessed', 'Workout session accessed', context, { sessionId })
  ...
  async startWorkoutSession(
    ...
    try {
      ...
      if (sessionResult.data.status === 'in_progress') {
        return this.createErrorResult('Session is already in progress', 'INVALID_STATE')
      }

      if (sessionResult.data.status === 'completed') {
        return this.createErrorResult('Session is already completed', 'INVALID_STATE')
      }
      ...
      if (result.length === 0) {
        return this.createErrorResult('Failed to start session', 'UPDATE_FAILED')
      }

      const updatedSession = this.mapWorkoutSessionFromDb(result[0])

      await this.logEvent('session_started', 'Workout session started', context, { sessionId })

      return this.createSuccessResult(updatedSession, 'Workout session started successfully')
    } catch (error) {
  ...
  async completeWorkoutSession(
    sessionId: string,
    data: { effortRating?: number; energyLevelAfter?: number; userNotes?: string },
    context: ServiceContext
  ): Promise<ServiceResult<WorkoutSession>> {
    try {
      this.validateContext(context)
      ...
      const actualDuration = sessionResult.data.startedAt 
        ? Math.round((Date.now() - new Date(sessionResult.data.startedAt).getTime()) / (1000 * 60))
        : null
      ...
      if (result.length === 0) {
        return this.createErrorResult('Failed to complete session', 'UPDATE_FAILED')
      }

      const completedSession = this.mapWorkoutSessionFromDb(result[0])

      await this.logEvent('session_completed', 'Workout session completed', context, { 
        sessionId, 
        duration: actualDuration 
      })

      return this.createSuccessResult(completedSession, 'Workout session completed successfully')
    } catch (error) {
  ...
  private buildWorkoutPlanFilters(filters: WorkoutPlanFilters = {}, context: ServiceContext) {
    const conditions: string[] = ['wp.is_active = true']
    const values: any[] = []
    let paramIndex = 1

    // Always filter by user or organization
    conditions.push(`(wp.user_id = (SELECT id FROM user_profiles WHERE clerk_user_id = $${paramIndex++})`)
    ...
    if (filters.search) {
      conditions.push(`(wp.name ILIKE $${paramIndex++} OR wp.description ILIKE $${paramIndex++})`)
  ...
  private buildWorkoutSessionFilters(filters: WorkoutSessionFilters = {}, context: ServiceContext) {
    const conditions: string[] = ['ws.is_active = true']
    const values: any[] = []
    let paramIndex = 1

    // Always filter by user or organization access
    conditions.push(`ws.user_id = (SELECT id FROM user_profiles WHERE clerk_user_id = $${paramIndex++})`)
    values.push(context.userId)
