...
class OfflineStorageService {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(this.stores.workouts)) {
          const workoutsStore = db.createObjectStore(this.stores.workouts, {
            keyPath: 'id',
          });
          workoutsStore.createIndex('sessionId', 'sessionId', {
            unique: false,
          });
          workoutsStore.createIndex('timestamp', 'timestamp', {
            unique: false,
          });
        ...
        if (!db.objectStoreNames.contains(this.stores.plans)) {
          const plansStore = db.createObjectStore(this.stores.plans, {
            keyPath: 'id',
          });
        ...
        if (!db.objectStoreNames.contains(this.stores.exercises)) {
          const exercisesStore = db.createObjectStore(this.stores.exercises, {
            keyPath: 'id',
          });
          exercisesStore.createIndex('sessionId', 'sessionId', {
            unique: false,
          });
          exercisesStore.createIndex('exerciseId', 'exerciseId', {
            unique: false,
          });
          exercisesStore.createIndex('timestamp', 'timestamp', {
            unique: false,
          });
        }
    ...
    return new Promise((resolve, reject) => {
      ...
      getRequest.onsuccess = () => {
        ...
        if (workoutData) {
          ...
          putRequest.onerror = () => {
            console.error(
              'Failed to mark workout as synced:',
              putRequest.error
            );
      ...
      getRequest.onerror = () => {
        console.error(
          'Failed to get workout for sync update:',
          getRequest.error
        );
   ...
   */
  async getExerciseDataForSession(
    sessionId: string
  ): Promise<OfflineExerciseData[]> {
    if (!this.db) {
      await this.initialize();
    }
  ...
  async saveSetting(key: string, value: any): Promise<void> {
    ...
    return this.putData(this.stores.settings, {
      key,
      value,
      timestamp: Date.now(),
    });
  ...
  async getStorageStats(): Promise<{
    ...
    const [workouts, plans, exercises, unsyncedWorkouts, queueSize] =
      await Promise.all([
  ...
  private async getAllData(storeName: string): Promise<any[]> {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        console.error(
          'Failed to get all data from store:',
          storeName,
          request.error
        );
  ...
  private async deleteData(storeName: string, key: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        console.error(
          'Failed to delete data from store:',
          storeName,
          request.error
        );
  ...
  private async getCount(storeName: string): Promise<number> {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        console.error(
          'Failed to get count from store:',
          storeName,
          request.error
        );
