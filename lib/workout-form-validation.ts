...
import type { FormValidationErrors, ValidationResult, StepValidator } from '@/types/workout-forms'

/**
 * Validates the basics step of the workout plan creation
 */
export const validateBasicsStep: StepValidator = (data: CustomPlanFormData): ValidationResult => {
  ...
  if (!data.sessionsPerWeek || data.sessionsPerWeek < 1 || data.sessionsPerWeek > 7) {
 ...
 */
export const validateScheduleStep: StepValidator = (data: CustomPlanFormData): ValidationResult => {
  const errors: FormValidationErrors = {}
  ...
  const totalWorkouts = Object.values(data.weeklySchedule || {}).reduce((total, weekSchedule: any) => {
 ...
 */
export const validateTemplatesStep: StepValidator = (data: CustomPlanFormData): ValidationResult => {
  const errors: FormValidationErrors = {}
  ...
  if (data.sessionTemplates) {
    const invalidTemplates = data.sessionTemplates.filter(template => 
      !template.name || template.name.trim().length < 2 ||
      template.estimatedDuration < 15 ||
      template.estimatedDuration > 180
    )

    if (invalidTemplates.length > 0) {
      errors.sessionTemplates = 'All templates must have valid names and durations (15-180 minutes)'
    }
  }
 ...
 */
export const validateCompleteForm = (data: CustomPlanFormData): ValidationResult => {
 ...
 */
export const canProgressToStep = (data: CustomPlanFormData, targetStep: string): boolean => {
  ...
  for (let i = 0; i < targetIndex; i++) {
    const validator = getStepValidator(stepOrder[i])
 ...
 */
export const sanitizeFormData = (data: CustomPlanFormData): CustomPlanFormData => {
