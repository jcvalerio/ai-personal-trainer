...
import {
  useFormState,
  useClearFormOnSubmit,
} from '@/components/workouts/create-manual/form-state-provider';
import {
  createWorkoutPlan,
  type ApiResponse,
...
export function useWorkoutPlanSubmission(
  options: UseWorkoutPlanSubmissionOptions = {}
) {
  ...
  const validateFormData = (data: CustomPlanFormData): string | null => {
    ...
    const invalidTemplates = data.sessionTemplates.filter(
      (template) =>
        !template.exerciseStructure || template.exerciseStructure.length === 0
    );
  ...
  const submitWorkoutPlan = async (
    customData?: Partial<CustomPlanFormData>
  ) => {
