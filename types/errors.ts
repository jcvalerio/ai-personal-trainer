...
export class AppErrorBuilder<T extends ErrorCategoryType = ErrorCategoryType> {
  ...
  withContext(context: Record<string, unknown>): this {
    this.error = {
      ...this.error,
      context: { ...this.error.context, ...context },
    };
    return this;
  }
...
export class ValidationErrorBuilder extends AppErrorBuilder<'validation'> {
  ...
  override build(): ValidationError {
    ...
    return {
      ...
      fields: this.fields,
      schemaViolations:
        this.schemaViolations.length > 0 ? this.schemaViolations : undefined,
    };
  }
}
...
export const ERROR_MESSAGES = {
  [ERROR_CODES.VALIDATION_FAILED]:
    'Validation failed for the provided data' as ErrorMessage,
  [ERROR_CODES.REQUIRED_FIELD_MISSING]:
    'Required field is missing' as ErrorMessage,
  [ERROR_CODES.INVALID_FORMAT]: 'Invalid format provided' as ErrorMessage,
  [ERROR_CODES.INVALID_CREDENTIALS]:
    'Invalid credentials provided' as ErrorMessage,
  [ERROR_CODES.TOKEN_EXPIRED]:
    'Authentication token has expired' as ErrorMessage,
  [ERROR_CODES.TOKEN_INVALID]:
    'Authentication token is invalid' as ErrorMessage,
  [ERROR_CODES.INSUFFICIENT_PERMISSIONS]:
    'Insufficient permissions for this action' as ErrorMessage,
  [ERROR_CODES.RESOURCE_FORBIDDEN]:
    'Access to this resource is forbidden' as ErrorMessage,
  [ERROR_CODES.CONNECTION_FAILED]:
    'Failed to connect to database' as ErrorMessage,
  [ERROR_CODES.QUERY_TIMEOUT]: 'Database query timed out' as ErrorMessage,
  [ERROR_CODES.CONSTRAINT_VIOLATION]:
    'Database constraint violation' as ErrorMessage,
  [ERROR_CODES.REQUEST_TIMEOUT]: 'Request timed out' as ErrorMessage,
  [ERROR_CODES.SERVICE_UNAVAILABLE]:
    'Service is currently unavailable' as ErrorMessage,
  [ERROR_CODES.RATE_LIMITED]: 'Rate limit exceeded' as ErrorMessage,
  [ERROR_CODES.BUSINESS_RULE_VIOLATION]:
    'Business rule violation' as ErrorMessage,
  [ERROR_CODES.RESOURCE_CONFLICT]: 'Resource conflict detected' as ErrorMessage,
  [ERROR_CODES.INVALID_STATE]:
    'Invalid state for this operation' as ErrorMessage,
  ...
  [ERROR_CODES.RESOURCE_EXHAUSTED]:
    'System resources are exhausted' as ErrorMessage,
} as const;
