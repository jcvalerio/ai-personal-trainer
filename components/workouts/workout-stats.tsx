    ...
    <Card className={cn('', className)}>
      <CardContent className='p-6'>
        <div className='flex items-center gap-4'>
          <div className={cn('rounded-lg p-3', colorClasses.icon)}>{icon}</div>
  ...
  return (
    <div className={cn('grid grid-cols-2 gap-4 lg:grid-cols-4', className)}>
      <StatCard
        ...
        trend={
          trends?.workouts
            ? { ...trends.workouts, label: 'from last month' }
            : undefined
        }
      ...
      <StatCard
        ...
        trend={
          trends?.streak
            ? { ...trends.streak, label: 'streak change' }
            : undefined
        }
...
export function ProgressOverview({
  ...
  const monthlyPercentage = Math.min(
    (monthlyStats.completed / monthlyStats.planned) * 100,
    100
  );
