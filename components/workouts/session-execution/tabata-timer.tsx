...
export function TabataTimer({
  ...
  const playSound = useCallback(
    (phase: TimerPhase | 'warning') => {
      ...
      gainNode.gain.exponentialRampToValueAtTime(
        0.01,
        ctx.currentTime + feedback.duration / 1000
      );
  ...
  const getNextPhaseConfig = useCallback(
    (currentPhase: TimerPhase, round: number, set: number) => {
      switch (currentPhase) {
        case 'ready':
          return {
  ...
  const tick = useCallback(() => {
    ...
    setTimerState((prevState) => {
      ...
      if (
        newTimeRemaining <= 3000 &&
        newTimeRemaining > 2900 &&
  ...
  const progressPercentage =
    timerState.phaseTotal > 0
      ? ((timerState.phaseTotal - timerState.timeRemaining) /
          timerState.phaseTotal) *
        100
      : 0;
  ...
  return (
    <Card className={`${className} transition-all duration-300`}>
      <CardContent
        className={`p-8 ${phaseStyles.background} ${phaseStyles.text} rounded-lg`}
      >
        {/* Exercise Information */}
        <div className='mb-6 text-center'>
          <h2 className='mb-2 text-2xl font-bold'>{exercise.name}</h2>
          <div className='text-lg opacity-90'>
            {t('timer.tabata.phase.' + timerState.phase)} - {t('set')}{' '}
          ...
          {!timerState.isRunning &&
            ...
            timerState.phase !== 'completed' && (
              <Button
                size='lg'
                onClick={startTimer}
                className='bg-white/20 px-8 text-lg text-white hover:bg-white/30'
              >
