...
const OfflineSessionExecutionContext = createContext<
  OfflineSessionExecutionContextType | undefined
>(undefined);
...
const DEFAULT_SESSION_SETTINGS: SessionSettings = {
  ...
  savePhotos: false,
  syncWearables: false,
  restPeriods: [60, 120, 180], // betweenSets, betweenExercises, betweenCircuits
};
  ...
  switch (action.type) {
    case 'START_SESSION':
      return {
        ...
        performance: {
          ...
          totalSets: action.payload.exercises.reduce(
            (sum: number, ex: SessionExercise) => sum + (ex.sets || 0),
            0
          ),
        },
      };
    ...
    case 'PAUSE_SESSION':
      return {
        ...
        activeTimer: state.activeTimer
          ? { ...state.activeTimer, isRunning: false, isPaused: true }
          : undefined,
      };
    ...
    case 'RESUME_SESSION':
      return {
        ...
        status: 'active',
        activeTimer: state.activeTimer
          ? { ...state.activeTimer, isRunning: true, isPaused: false }
          : undefined,
      };
    ...
    case 'COMPLETE_SET':
      ...
      const isExerciseComplete =
        updatedExercise.completedSets.length >= currentExercise.sets;
      ...
      return {
        ...
        performance: {
          ...
          totalVolume:
            state.performance.totalVolume +
            (action.payload.setData.weight || 0) *
              (action.payload.setData.reps || 0),
        },
      };
    ...
    case 'UPDATE_TIMER':
      return {
        ...
        elapsedTime:
          state.elapsedTime + (action.payload.timerState.elapsedTime || 0),
      };
...
export function OfflineSessionExecutionProvider({
  children,
}: {
  ...
  const currentExercise: ExecutingExercise | null =
  ...
  const startSession = useCallback(
    (exercises: SessionExercise[], settings?: Partial<SessionSettings>) => {
      const sessionId = `session_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

      const executingExercises: ExecutingExercise[] = exercises.map(
        (exercise) => ({
          ...exercise,
          completedSets: [],
          startTime: null,
  ...
  const completeSession = useCallback(async () => {
    ...
    console.log(
      'Session completed:',
      state.sessionId,
      offlineStatus.isOffline ? '(saved offline)' : '(will sync)'
    );
  ...
  const syncOfflineData = useCallback(async () => {
    if (offlineStatus.isOffline || isSyncing) {
      return;
    }
...
export function useOfflineSessionExecution() {
  ...
  if (!context) {
    throw new Error(
      'useOfflineSessionExecution must be used within an OfflineSessionExecutionProvider'
    );
  }
  return context;
}
