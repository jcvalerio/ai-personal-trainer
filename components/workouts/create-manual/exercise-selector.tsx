...
function ExerciseCard({ 
  ...
  const difficultyConfig = DIFFICULTY_LEVELS.find(d => d.value === exercise.difficulty)
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {difficultyConfig && (
                <Badge variant="outline" className="text-xs">
                  <div className={cn('w-2 h-2 rounded-full mr-1', difficultyConfig.color)} />
          ...
          <Button
            ...
            onClick={() => isSelected ? onDeselect(exercise.id) : onSelect(exercise)}
            className="ml-2"
          >
...
function ExerciseConfigPanel({ 
  ...
  const updateConfig = useCallback((updates: Partial<ExerciseConfig>) => {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        ...
        {exercise.type !== 'isometric' ? (
          <div>
            ...
            <Input
              ...
              max="50"
              value={config.reps || ''}
              onChange={(e) => updateConfig({ reps: parseInt(e.target.value) || undefined })}
              placeholder="8-12"
              className="h-8"
            />
          </div>
        ...
        {/* Weight */}
        {exercise.equipment.some(eq => ['dumbbells', 'barbell', 'machines'].includes(eq)) && (
          <div>
            <label className="text-xs font-medium text-gray-700 block mb-1">
              Weight (lbs)
            </label>
            <Input
              ...
              step="5"
              value={config.weight || ''}
              onChange={(e) => updateConfig({ weight: parseInt(e.target.value) || undefined })}
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(value) => updateConfig({ restTime: parseInt(value) })}
          >
...
export function ExerciseSelector({ 
  ...
  const handleSelectExercise = useCallback((exercise: Exercise) => {
    if (selectedExercises.length >= maxExercises) {return}
  ...
  const handleDeselectExercise = useCallback((exerciseId: string) => {
    onExercisesChange(selectedExercises.filter(config => config.exerciseId !== exerciseId))
  }, [selectedExercises, onExercisesChange])

  const handleUpdateExerciseConfig = useCallback((updatedConfig: ExerciseConfig) => {
    onExercisesChange(
      selectedExercises.map(config => 
        config.exerciseId === updatedConfig.exerciseId ? updatedConfig : config
      )
    )
  }, [selectedExercises, onExercisesChange])

  const getExerciseById = useCallback((id: string) => 
    MOCK_EXERCISES.find(ex => ex.id === id), [])
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center justify-between text-sm text-gray-600">
                ...
                {selectedExercises.length > 0 && (
                  <span>{selectedExercises.length}/{maxExercises} selected</span>
                )}
              </div>
            </CardContent>
          </Card>
