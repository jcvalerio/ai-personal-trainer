...
export function OfflineIndicator({ 
  ...
  if (offlineStatus.isOnline && (!storageStats || storageStats.unsyncedWorkouts === 0)) {
    return null
  }
  ...
  return (
    <Card className={`${positionClasses[position]} ${className} w-80 shadow-lg border-2 ${
      offlineStatus.isOffline 
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-sm">
              {offlineStatus.isOffline ? 'You\'re Offline' : 
      ...
      <CardContent className="pt-0 space-y-3">
        ...
        <div className="flex flex-wrap gap-1">
          <Badge variant={offlineStatus.isOffline ? 'destructive' : 'secondary'}>
        ...
        {storageStats && (
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              <div className="flex justify-between">
                <span>Unsynced:</span>
                <span className={`font-medium ${storageStats.unsyncedWorkouts > 0 ? 'text-yellow-600' : 'text-green-600'}`}>
        ...
        <div className="flex gap-2">
          {offlineStatus.isOnline && storageStats?.unsyncedWorkouts && storageStats.unsyncedWorkouts > 0 && (
            <Button
              size="sm"
              variant="outline"
        ...
        {(offlineStatus.lastOnlineTime || offlineStatus.lastOfflineTime) && (
          <div className="text-xs text-gray-500">
            {offlineStatus.isOffline && offlineStatus.lastOfflineTime && (
              <div>Offline since: {offlineStatus.lastOfflineTime.toLocaleTimeString()}</div>
            )}
            {offlineStatus.isOnline && offlineStatus.lastOnlineTime && (
              <div>Connected: {offlineStatus.lastOnlineTime.toLocaleTimeString()}</div>
            )}
          </div>
        )}
      </CardContent>
  ...
  return (
    <Alert className={`border-yellow-200 bg-yellow-50 ${className}`}>
      ...
      <AlertDescription className="text-sm">
        <strong>You're offline.</strong> Your workout data is being saved locally and will sync when you reconnect.
      </AlertDescription>
    </Alert>
  )
}
...
export function ConnectionQualityIndicator({ className = '' }: { className?: string }) {
  const { isOnline, isSlowConnection, isUnstableConnection, effectiveType } = useOfflineStatus()

  if (!isOnline) {return null}

  const getConnectionColor = () => {
    if (isUnstableConnection) {return 'text-red-500'}
    if (isSlowConnection) {return 'text-yellow-500'}
    return 'text-green-500'
  }

  const getConnectionText = () => {
    if (isUnstableConnection) {return 'Poor'}
