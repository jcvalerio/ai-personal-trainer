...
import {
  ...
  SignUpPage,
  OnboardingPage,
} from '../utils/page-objects/auth.pages';
...
test.describe('Authentication Flow', () => {
  ...
  test('should navigate between sign-in and sign-up pages', async ({
    ...
    // Navigate to sign-up
    if (
      await signInPage.elementExists(
        'a[href*="sign-up"], a:has-text("Sign up")'
      )
    ) {
      ...
      if (
        await signUpPage.elementExists(
          'a[href*="sign-in"], a:has-text("Sign in")'
        )
      ) {
