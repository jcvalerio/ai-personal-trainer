...
export class I18nPage extends BasePage {
  ...
  private readonly languageToggle = () =>
    this.page.locator(
      '[data-testid="language-toggle"], button:has-text("ES"), button:has-text("EN")'
    );
  private readonly langEsButton = () =>
    this.page.locator('button:has-text("ES"), a:has-text("Español")');
  private readonly langEnButton = () =>
    this.page.locator('button:has-text("EN"), a:has-text("English")');

  // Content Locators for Translation Verification
  private readonly pageTitle = () =>
    this.page.locator('h1, [data-testid="page-title"]');
  private readonly navigationItems = () =>
    this.page.locator('nav a, [data-testid="nav-item"]');
  private readonly mainContent = () =>
    this.page.locator('main, [data-testid="main-content"]');
  ...
  private readonly validationMessages = () =>
    this.page.locator('.error-message, [data-testid="error"], .field-error');
  private readonly requiredFields = () =>
    this.page.locator('input[required], select[required], textarea[required]');

  // Timer Interface Locators
  private readonly timerControls = () =>
    this.page.locator('[data-testid="timer"] button, .timer-controls button');
  private readonly timerLabels = () =>
    this.page.locator(
      '[data-testid="timer"] span, .timer-display, .phase-label'
    );

  // Date/Time Locators
  private readonly dateElements = () =>
    this.page.locator('[data-testid*="date"], .date, time');
  private readonly timeElements = () =>
    this.page.locator('[data-testid*="time"], .timestamp');
  ...
  async getCurrentLanguage(): Promise<string> {
    ...
    try {
      const activeLanguage = await this.page
        .locator('[data-testid="active-language"], .active-lang')
        .textContent();
  ...
  async switchLanguage(lang: 'en' | 'es'): Promise<void> {
    try {
      ...
      if (
        lang === 'es' &&
        (await this.elementExists(
          'button:has-text("ES"), a:has-text("Español")'
        ))
      ) {
      ...
      if (
        lang === 'en' &&
        (await this.elementExists(
          'button:has-text("EN"), a:has-text("English")'
        ))
      ) {
      ...
      console.log(
        `Language switch to ${lang} attempted via URL:`,
        error.message
      );
  ...
  async testBasicNavigation(): Promise<boolean> {
    try {
      ...
      const navigationWorked =
        dashboardUrl !== currentUrl && dashboardUrl.includes('dashboard');
