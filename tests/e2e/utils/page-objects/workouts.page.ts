...
export class WorkoutsPage extends BasePage {
  // Locators
  private readonly workoutsList = () => this.page.locator('[data-testid="workouts-list"]')
  private readonly createWorkoutButton = () => this.page.locator('button:has-text("Create"), a:has-text("New Workout")')
  private readonly searchInput = () => this.page.locator('input[placeholder*="Search" i], input[type="search"]')
  private readonly filterDropdown = () => this.page.locator('select[data-testid="filter"], select:has(option)')
  private readonly workoutCards = () => this.page.locator('[data-testid="workout-card"]')
  ...
  async selectWorkout(index: number): Promise<void> {
    ...
    if (index >= cards.length) {
      throw new Error(`Workout index ${index} is out of range. Found ${cards.length} workouts.`)
    }
  ...
  async selectWorkoutByName(name: string): Promise<void> {
    const workoutCard = this.page.locator(`[data-testid="workout-card"]:has-text("${name}")`)
  ...
  async createNewWorkout(): Promise<void> {
    if (await this.elementExists('button:has-text("Create"), a:has-text("New Workout")')) {
  ...
  async searchWorkouts(query: string): Promise<void> {
    if (await this.elementExists('input[placeholder*="Search" i], input[type="search"]')) {
  ...
  async filterByCategory(category: string): Promise<void> {
    if (await this.elementExists('select[data-testid="filter"], select:has(option)')) {
  ...
  async verifyWorkoutsDisplayed(): Promise<void> {
    ...
    const hasWorkouts = await this.elementExists('[data-testid="workout-card"]')
...
export class WorkoutPlansPage extends BasePage {
  // Locators
  private readonly plansList = () => this.page.locator('[data-testid="plans-list"]')
  private readonly createPlanButton = () => this.page.locator('button:has-text("Create Plan"), a:has-text("New Plan")')
  private readonly planCards = () => this.page.locator('[data-testid="plan-card"]')
  private readonly searchInput = () => this.page.locator('input[placeholder*="Search" i], input[type="search"]')
  ...
  async selectPlan(index: number): Promise<void> {
    ...
    if (index >= cards.length) {
      throw new Error(`Plan index ${index} is out of range. Found ${cards.length} plans.`)
    }
  ...
  async createNewPlan(): Promise<void> {
    if (await this.elementExists('button:has-text("Create Plan"), a:has-text("New Plan")')) {
...
export class WorkoutGenerationPage extends BasePage {
  // Locators
  private readonly fitnessLevelSelect = () => this.page.locator('select[name="fitnessLevel"], select[data-testid="fitness-level"]')
  private readonly durationInput = () => this.page.locator('input[name="duration"], input[data-testid="duration"]')
  private readonly equipmentCheckboxes = () => this.page.locator('input[type="checkbox"][name*="equipment"]')
  private readonly muscleGroupsSelect = () => this.page.locator('select[name="muscleGroups"], [data-testid="muscle-groups"]')
  private readonly generateButton = () => this.page.locator('button:has-text("Generate"), button[type="submit"]')
  private readonly generatedWorkout = () => this.page.locator('[data-testid="generated-workout"]')
  ...
  async fillGenerationForm(options: {
    ...
    // Set fitness level
    if (options.fitnessLevel && await this.elementExists('select[name="fitnessLevel"], select[data-testid="fitness-level"]')) {
      await this.fitnessLevelSelect().selectOption(options.fitnessLevel)
    }

    // Set duration
    if (options.duration && await this.elementExists('input[name="duration"], input[data-testid="duration"]')) {
    ...
    if (options.muscleGroups && options.muscleGroups.length > 0) {
      ...
      } else if (await this.elementExists('input[type="checkbox"][name*="muscle"]')) {
        const muscleCheckboxes = this.page.locator('input[type="checkbox"][name*="muscle"]')
  ...
  async generateWorkout(): Promise<void> {
    ...
    await this.waitForLoadingComplete()
    await this.waitForElement('[data-testid="generated-workout"], .workout-result', 30000)
  }

  /**
   * Complete workout generation flow
   */
  async completeWorkoutGeneration(options: {
    fitnessLevel?: string
    duration?: number
    equipment?: string[]
    muscleGroups?: string[]
  ...
  async isWorkoutGenerated(): Promise<boolean> {
    return await this.elementExists('[data-testid="generated-workout"], .workout-result')
  }
  ...
  async verifyGenerationForm(): Promise<void> {
    ...
    const hasFormElements = await Promise.race([
      this.elementExists('select[name="fitnessLevel"], select[data-testid="fitness-level"]'),
      this.elementExists('input[name="duration"], input[data-testid="duration"]'),
