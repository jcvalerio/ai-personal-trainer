...
export class DashboardPage extends BasePage {
  ...
  private readonly welcomeMessage = () =>
    this.page.locator('[data-testid="welcome-message"], h1, .welcome').first();
  private readonly workoutSummary = () =>
    this.page.locator('[data-testid="workout-summary"]');
  private readonly quickActions = () =>
    this.page.locator('[data-testid="quick-actions"]');
  private readonly recentWorkouts = () =>
    this.page.locator('[data-testid="recent-workouts"]');
  private readonly progressChart = () =>
    this.page.locator('[data-testid="progress-chart"]');
  private readonly navigationMenu = () =>
    this.page.locator('[data-testid="navigation"], nav');

  // Navigation links
  private readonly workoutsLink = () =>
    this.page.locator('a[href*="/workouts"], a:has-text("Workouts")');
  private readonly exercisesLink = () =>
    this.page.locator('a[href*="/exercises"], a:has-text("Exercises")');
  private readonly progressLink = () =>
    this.page.locator('a[href*="/progress"], a:has-text("Progress")');

  // Quick action buttons
  private readonly startWorkoutButton = () =>
    this.page.locator(
      'button:has-text("Start Workout"), a:has-text("Start Workout")'
    );
  private readonly createPlanButton = () =>
    this.page.locator(
      'button:has-text("Create Plan"), a:has-text("Create Plan")'
    );
  private readonly generateWorkoutButton = () =>
    this.page.locator('button:has-text("Generate"), a:has-text("AI Workout")');
  ...
  async getWelcomeMessage(): Promise<string> {
    if (
      await this.elementExists('[data-testid="welcome-message"], h1, .welcome')
    ) {
  ...
  async getWorkoutSummaryData(): Promise<{
    ...
    try {
      const totalWorkoutsElement = this.page.locator(
      ...
      const weeklyGoalElement = this.page.locator(
  ...
  async generateAIWorkout(): Promise<void> {
    if (
      await this.elementExists(
        'button:has-text("Generate"), a:has-text("AI Workout")'
      )
    ) {
  ...
  async getRecentWorkouts(): Promise<string[]> {
    ...
    const workoutElements = await this.page
      .locator('[data-testid="recent-workouts"] [data-testid="workout-item"]')
      .all();
  ...
  async hasNavigationMenu(): Promise<boolean> {
    return await this.elementExists(
      '[data-testid="navigation"], nav, [data-testid="sidebar"]'
    );
  }
  ...
  async getNavigationItems(): Promise<string[]> {
    ...
    const navItems = await this.page
      .locator('nav a, [data-testid="navigation"] a, [data-testid="nav-item"]')
      .all();
    const items: string[] = [];
  ...
  async hasProgressChart(): Promise<boolean> {
    return await this.elementExists(
      '[data-testid="progress-chart"], .chart, svg'
    );
  }
  ...
  async waitForDashboardData(): Promise<void> {
    ...
    await Promise.race([
      ...
      console.log(
        'Dashboard data sections may not be present - continuing with test'
      );
    });
  }
  ...
  async getDashboardStats(): Promise<any> {
    ...
    try {
      ...
      const statsElements = await this.page
        .locator('[data-testid*="stat"], .stat-card, [data-testid*="metric"]')
        .all();
