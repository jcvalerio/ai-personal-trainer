...
export class WorkoutSessionPage extends BasePage {
  ...
  private readonly sessionsList = () =>
    this.page.locator('[data-testid="sessions-list"]');
  private readonly startSessionButton = () =>
    this.page.locator('button:has-text("Start"), a:has-text("New Session")');
  private readonly activeSessionLink = () =>
    this.page.locator('a[href*="/sessions/"], [data-testid="active-session"]');

  // Session Page Locators
  private readonly sessionContainer = () =>
    this.page.locator(
      '[data-testid="session-container"], [data-testid="workout-session"]'
    );
  private readonly exerciseList = () =>
    this.page.locator('[data-testid="exercise-list"]');
  private readonly currentExercise = () =>
    this.page.locator('[data-testid="current-exercise"]');
  private readonly exerciseCards = () =>
    this.page.locator('[data-testid="exercise-card"]');

  // Timer Locators
  private readonly timer = () =>
    this.page.locator(
      '[data-testid="timer"], .timer, [data-testid="workout-timer"]'
    );
  private readonly timerDisplay = () =>
    this.page.locator('[data-testid="timer-display"], .timer-display');
  private readonly playButton = () =>
    this.page.locator(
      'button[data-testid="play"], button:has([data-testid="play-icon"])'
    );
  private readonly pauseButton = () =>
    this.page.locator(
      'button[data-testid="pause"], button:has([data-testid="pause-icon"])'
    );
  private readonly resetButton = () =>
    this.page.locator('button[data-testid="reset"], button:has-text("Reset")');

  // Tabata Specific Locators
  private readonly tabataContainer = () =>
    this.page.locator(
      '[data-testid="tabata-timer"], [data-testid="tabata-container"]'
    );
  private readonly workPhase = () =>
    this.page.locator('[data-testid="work-phase"], .work-phase');
  private readonly restPhase = () =>
    this.page.locator('[data-testid="rest-phase"], .rest-phase');
  private readonly roundCounter = () =>
    this.page.locator(
      '[data-testid="round-counter"], [data-testid="cycle-counter"]'
    );
  private readonly startTabataButton = () =>
    this.page.locator(
      'button:has-text("Start Tabata"), button[data-testid="start-tabata"]'
    );
  private readonly stopTabataButton = () =>
    this.page.locator(
      'button:has-text("Stop"), button[data-testid="stop-tabata"]'
    );

  // Progress and Control Locators
  private readonly completeButton = () =>
    this.page.locator(
      'button:has-text("Complete"), button[data-testid="mark-complete"]'
    );
  private readonly nextExerciseButton = () =>
    this.page.locator(
      'button:has-text("Next"), button[data-testid="next-exercise"]'
    );
  private readonly previousExerciseButton = () =>
    this.page.locator(
      'button:has-text("Previous"), button[data-testid="previous-exercise"]'
    );
  private readonly progressBar = () =>
    this.page.locator('[data-testid="progress-bar"], .progress-bar');
  private readonly finishSessionButton = () =>
    this.page.locator(
      'button:has-text("Finish"), button[data-testid="finish-session"]'
    );
  private readonly saveProgressButton = () =>
    this.page.locator(
      'button:has-text("Save"), button[data-testid="save-progress"]'
    );

  // Status and Message Locators
  private readonly completionMessage = () =>
    this.page.locator(
      '[data-testid="completion-message"], .completion-message'
    );
  private readonly offlineMessage = () =>
    this.page.locator('[data-testid="offline-message"], .offline-indicator');
  private readonly sessionStats = () =>
    this.page.locator('[data-testid="session-stats"], .session-statistics');
  ...
  async navigateToSessionByType(workoutType: string): Promise<void> {
    ...
    const sessionWithType = this.page.locator(
      `[data-testid="session-card"]:has-text("${workoutType}")`
    );
  ...
  async verifySessionsDisplayed(): Promise<void> {
    ...
    const hasSessions = await this.elementExists(
      '[data-testid="session-card"]'
    );
    const hasEmptyState = await this.elementExists(
      '[data-testid="empty-state"], .empty-state'
    );
    const hasCreateButton = await this.elementExists(
      'button:has-text("Start"), a:has-text("New Session")'
    );
  ...
  async verifySessionElements(): Promise<void> {
    ...
    const hasExercises = await this.elementExists(
      '[data-testid="exercise-list"], [data-testid="current-exercise"]'
    );
  ...
  async verifyTimerControls(): Promise<boolean> {
    const hasPlay = await this.elementExists(
      'button[data-testid="play"], button:has([data-testid="play-icon"])'
    );
    const hasPause = await this.elementExists(
      'button[data-testid="pause"], button:has([data-testid="pause-icon"])'
    );
    const hasReset = await this.elementExists(
      'button[data-testid="reset"], button:has-text("Reset")'
    );
  ...
  async verifyWorkRestCycles(): Promise<boolean> {
    const hasWorkPhase = await this.elementExists(
      '[data-testid="work-phase"], .work-phase'
    );
    const hasRestPhase = await this.elementExists(
      '[data-testid="rest-phase"], .rest-phase'
    );
    const hasPhaseIndicator = await this.elementExists(
      '.phase-indicator, [data-testid="phase"]'
    );
  ...
  async verifyIntervalElements(): Promise<boolean> {
    const hasIntervalTimer = await this.elementExists(
      '[data-testid="interval-timer"]'
    );
    const hasIntervalPhases = await this.elementExists(
  ...
  async isExerciseComplete(): Promise<boolean> {
    return await this.elementExists(
      '[data-testid="exercise-completed"], .exercise-complete'
    );
  }
  ...
  async getProgressPercentage(): Promise<number> {
    if (
      await this.elementExists('[data-testid="progress-bar"], .progress-bar')
    ) {
    ...
    const completed = await this.page
      .locator('[data-testid="exercise-completed"]')
      .count();
  ...
  async checkForOfflineMessage(): Promise<boolean> {
    return await this.elementExists(
      '[data-testid="offline-message"], .offline-indicator'
    );
  }
  ...
  async verifyWorkoutTypeElements(workoutType: string): Promise<boolean> {
    switch (workoutType.toLowerCase()) {
      ...
      case 'strength':
        return await this.elementExists(
          '[data-testid="weight-input"], [data-testid="reps-input"]'
        );
      case 'cardio':
        return await this.elementExists(
          '[data-testid="distance-tracker"], [data-testid="pace-tracker"]'
        );
      default:
