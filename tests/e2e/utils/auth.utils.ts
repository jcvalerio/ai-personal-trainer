...
export class AuthUtils extends BasePage {
  ...
  async signIn(user?: Partial<TestUser>): Promise<void> {
    ...
    // Fill email field
    const emailField = this.page.locator('input[name="identifier"], input[type="email"]').first()
    ...
    const signInButton = this.page.locator('button[type="submit"], button:has-text("Sign in")').first()
  ...
  async signOut(): Promise<void> {
    ...
    try {
      ...
      if (userMenuFound) {
        ...
        const signOutButton = this.page.locator('button:has-text("Sign out"), [data-testid="sign-out"]').first()
  ...
  async verifyAuthenticated(): Promise<void> {
    ...
    if (!authenticated && !currentUrl.includes('/sign-in') && !currentUrl.includes('/sign-up')) {
      authenticated = true
    }
  ...
  async getAuthState(): Promise<AuthState> {
    try {
      ...
      const email = await this.page.evaluate(() => {
        return (window as any)?.Clerk?.user?.primaryEmailAddress?.emailAddress || null
      }).catch(() => null)
  ...
  private async fillOnboardingForm(): Promise<void> {
    ...
    const fields = [
      { selector: 'input[name="firstName"], input[placeholder*="first name" i]', value: this.testUser.firstName || 'Claude' },
      { selector: 'input[name="lastName"], input[placeholder*="last name" i]', value: this.testUser.lastName || 'Test' },
      { selector: 'select[name="fitnessLevel"], select[data-testid="fitness-level"]', value: 'intermediate' },
      { selector: 'input[name="age"], input[type="number"]', value: '30' },
      { selector: 'select[name="goals"], select[data-testid="fitness-goals"]', value: 'general_fitness' }
    ]
    ...
    const submitButton = this.page.locator('button[type="submit"], button:has-text("Complete"), button:has-text("Continue")').first()
    
    if (await this.elementExists('button[type="submit"], button:has-text("Complete"), button:has-text("Continue")')) {
      await Promise.all([
  ...
  async createTestUser(userData: TestUser): Promise<void> {
    if (process.env.TEST_CREATE_NEW_USERS !== 'true') {
      throw new Error('Test user creation is disabled. Set TEST_CREATE_NEW_USERS=true to enable.')
    }
    ...
    await this.fillField('input[name="emailAddress"], input[type="email"]', userData.email)
    await this.fillField('input[name="password"], input[type="password"]', userData.password)
    ...
    // Submit sign-up form
    const signUpButton = this.page.locator('button[type="submit"], button:has-text("Sign up")').first()
