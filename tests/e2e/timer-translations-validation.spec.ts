...
test.describe('Timer Interface Translations Validation', () => {
  test('should have comprehensive English timer translations', async ({ page }) => {
    ...
    if (messagesResponse.ok()) {
      ...
      expect(messages.workouts.session.timer.display.timeRemaining).toBe('Time Remaining')
      
      // Verify notifications exist
      expect(messages.workouts.session.timer.notifications).toBeDefined()
      expect(messages.workouts.session.timer.notifications.workoutStart).toBe('Workout Starting')
      expect(messages.workouts.session.timer.notifications.workoutComplete).toBe('Workout Complete!')
      
      console.log('✅ English timer translations validated successfully')
    } else {
      console.log('ℹ️ API endpoint not available, testing translation structure in code')
  ...
  test('should have comprehensive Spanish timer translations', async ({ page }) => {
    ...
    if (messagesResponse.ok()) {
      ...
      expect(messages.workouts.session.timer.tabata).toBeDefined()
      expect(messages.workouts.session.timer.tabata.title).toBe('Cronómetro Tabata')
      expect(messages.workouts.session.timer.tabata.phase.work).toBe('Trabajo')
      expect(messages.workouts.session.timer.tabata.phase.rest).toBe('Descanso')
      
      // Verify display translations exist in Spanish
      expect(messages.workouts.session.timer.display).toBeDefined()
      expect(messages.workouts.session.timer.display.minutes).toBe('min')
      expect(messages.workouts.session.timer.display.seconds).toBe('seg')
      expect(messages.workouts.session.timer.display.timeRemaining).toBe('Tiempo Restante')
      
      // Verify notifications exist in Spanish
      expect(messages.workouts.session.timer.notifications).toBeDefined()
      expect(messages.workouts.session.timer.notifications.workoutStart).toBe('Iniciando Entrenamiento')
      expect(messages.workouts.session.timer.notifications.workoutComplete).toBe('¡Entrenamiento Completado!')
      
      console.log('✅ Spanish timer translations validated successfully')
    } else {
      console.log('ℹ️ API endpoint not available, testing Spanish page structure')
  ...
  test('should verify translation file structure is correct', async ({ page }) => {
    // Test that both language files exist and have the expected structure
    await page.goto('/')
    await page.waitForLoadState('networkidle')
    
    // Read English translation file structure
    const enResponse = await page.request.get('/messages/en.json').catch(() => null)
    const esResponse = await page.request.get('/messages/es.json').catch(() => null)
    ...
    if (enResponse?.ok() && esResponse?.ok()) {
      ...
      const enControlsKeys = Object.keys(enMessages.workouts.session.timer.controls)
      const esControlsKeys = Object.keys(esMessages.workouts.session.timer.controls)
      expect(enControlsKeys.length).toBe(esControlsKeys.length)
      
      const enDisplayKeys = Object.keys(enMessages.workouts.session.timer.display)
      const esDisplayKeys = Object.keys(esMessages.workouts.session.timer.display)
      expect(enDisplayKeys.length).toBe(esDisplayKeys.length)
      
      const enNotificationKeys = Object.keys(enMessages.workouts.session.timer.notifications)
      const esNotificationKeys = Object.keys(esMessages.workouts.session.timer.notifications)
      ...
      console.log(`Control keys: ${enControlsKeys.length}, Display keys: ${enDisplayKeys.length}, Notification keys: ${enNotificationKeys.length}`)
      
    } else {
      console.log('ℹ️ Translation files not directly accessible, testing page functionality')
  ...
  test('should verify enhanced timer translations coverage', async ({ page }) => {
    await page.goto('/en')
    await page.waitForLoadState('networkidle')
    ...
    const messagesContent = await page.evaluate(() => {
      ...
      if (typeof window !== 'undefined' && (window as any).__NEXT_DATA__) {
        return (window as any).__NEXT_DATA__.props?.pageProps?.messages || null
      }
      return null
    }).catch(() => null)
    ...
    if (messagesContent) {
      ...
      console.log('ℹ️ Messages not accessible in page context, testing basic functionality')
    }
