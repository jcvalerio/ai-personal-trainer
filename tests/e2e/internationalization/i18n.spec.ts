...
test.describe('Internationalization (i18n) - English/Spanish', () => {
  ...
  test('should load application in default language (English)', async ({
    page,
  }) => {
  ...
  test('should persist language preference across navigation', async ({
    page,
  }) => {
    await i18nPage.navigate();
  ...
  test('should translate workout content correctly', async ({ page }) => {
    ...
    for (const [english, spanish] of Object.entries(workoutTranslations)) {
      const hasEnglish = englishWorkoutTerms.some((text) =>
        text.toLowerCase().includes(english.toLowerCase())
      );
      const hasSpanish = spanishWorkoutTerms.some((text) =>
        text.toLowerCase().includes(spanish.toLowerCase())
      );
  ...
  test('should translate timer interface correctly', async ({ page }) => {
    ...
    try {
      ...
      expect(JSON.stringify(englishTimerTerms)).not.toBe(
        JSON.stringify(spanishTimerTerms)
      );
      ...
      for (const [english, spanish] of Object.entries(timerTranslations)) {
        const hasEnglish = englishTimerTerms.some((text) =>
          text.toLowerCase().includes(english.toLowerCase())
        );
        const hasSpanish = spanishTimerTerms.some((text) =>
          text.toLowerCase().includes(spanish.toLowerCase())
        );
      ...
      console.log(
        'Timer interface translation test completed with limitations:',
        error.message
      );
    }
  });
  ...
  test('should translate form validation messages', async ({ page }) => {
    ...
    try {
      ...
      if (
        englishValidationMessages.length > 0 &&
        spanishValidationMessages.length > 0
      ) {
        expect(JSON.stringify(englishValidationMessages)).not.toBe(
          JSON.stringify(spanishValidationMessages)
        );
  ...
  test('should handle language switching with page refresh', async ({
    page,
  }) => {
    await i18nPage.navigate();
  ...
  test('should translate navigation menu items', async ({ page }) => {
    ...
    expect(JSON.stringify(englishNavItems)).not.toBe(
      JSON.stringify(spanishNavItems)
    );
  ...
  test('should handle direct URL access with language prefix', async ({
    page,
  }) => {
  ...
  test('should maintain functionality across language switches', async ({
    page,
  }) => {
    await i18nPage.navigate();
...
test.describe('i18n Edge Cases and Error Handling', () => {
  ...
  test('should handle missing translation keys', async ({ page }) => {
    ...
    if (missingTranslations.length > 0) {
      console.log(
        'Missing translations found (may need attention):',
        missingTranslations.slice(0, 5)
      );
