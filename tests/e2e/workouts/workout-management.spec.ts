...
test.describe('AI Workout Generation', () => {
  ...
  test('should generate workout with basic parameters', async ({ page }) => {
    ...
    try {
      ...
      const workoutContent =
        await workoutGenerationPage.getGeneratedWorkoutContent();
      expect(workoutContent.length).toBeGreaterThan(0);
    } catch (error) {
      console.log(
        'AI workout generation failed (may be expected if API not available):',
        error.message
      );
    }
  });
  ...
  test('should handle equipment selection', async ({ page }) => {
    ...
    try {
      const availableEquipment =
        await workoutGenerationPage.getAvailableEquipment();
  ...
  test('should handle generation errors gracefully', async ({ page }) => {
    ...
    try {
      await workoutGenerationPage.page
...
test.describe('Workout Data Integration', () => {
  ...
  test('should create workout plan via API', async ({ page }) => {
    ...
    try {
      ...
      console.log(
        'Workout plan creation via API not available:',
        error.message
      );
    }
  });
  ...
  test('should create workout session via API', async ({ page }) => {
    ...
    try {
      ...
      console.log(
        'Workout session creation via API not available:',
        error.message
      );
    }
  });
