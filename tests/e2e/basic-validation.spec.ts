...
test.describe('Basic Validation Tests', () => {
  ...
  test('should validate local server accessibility', async ({ page }) => {
    ...
    try {
      ...
      if (response && response.ok()) {
        ...
        console.log('⚠️ Local development server returned error:', response?.status())
        // Don't fail the test, just log the issue
      }
    } catch (error) {
      console.log('⚠️ Could not connect to local development server:', error.message)
  ...
  test('should validate test utilities can be imported', async () => {
    ...
    const { AuthUtils, TestDataUtils, BasePage } = await import('./utils/index')
