...
test.describe('Progress Tracking', () => {
  ...
  test('should display progress overview', async ({ page }) => {
    ...
    if (hasOverview) {
      ...
      const hasEmptyState = await progressPage.elementExists(
  ...
  test('should display workout statistics if available', async ({ page }) => {
    ...
    const statsElements = page.locator(
      '[data-testid="workout-stats"], .workout-stats, .stat-card'
    );
  ...
  test('should display progress charts if available', async ({ page }) => {
    ...
    if (hasCharts) {
      ...
      const chartElements = page.locator(
        '[data-testid="progress-chart"], .chart, svg, canvas'
      );
  ...
  test('should handle time period selection if available', async ({ page }) => {
    ...
    const timeFilters = page.locator(
      '[data-testid="time-filter"], select[data-testid="period"]'
    );
  ...
  test('should display workout history if available', async ({ page }) => {
    ...
    const historyElements = page.locator(
      '[data-testid="workout-history"], .workout-history'
    );
  ...
  test('should handle goal tracking if available', async ({ page }) => {
    ...
    const goalElements = page.locator(
      '[data-testid="goals"], .goals, [data-testid="goal-progress"]'
    );
    ...
    if ((await goalElements.count()) > 0) {
      ...
      for (const goal of goals) {
        ...
        if (text?.trim()) {
          ...
          const progressIndicators = goal.locator(
            '[data-testid="progress"], .progress, .progress-bar'
          );
  ...
  test('should display personal records if available', async ({ page }) => {
    ...
    const prElements = page.locator(
      '[data-testid="personal-records"], .personal-records, [data-testid="pr"]'
    );
...
test.describe('Progress Data Interaction', () => {
  ...
  test('should handle workout completion tracking', async ({ page }) => {
    ...
    const completedWorkouts = page.locator(
      '[data-testid="completed-workouts"], .completed-workouts'
    );

    if ((await completedWorkouts.count()) > 0) {
      const workoutList = completedWorkouts.locator(
  ...
  test('should export progress data if available', async ({ page }) => {
    ...
    const exportButtons = page.locator(
      'button:has-text("Export"), button[data-testid="export"]'
    );
  ...
  test('should handle progress photo uploads if available', async ({
    page,
  }) => {
    await progressPage.goto('/progress');
    await progressPage.waitForPageLoad();
    ...
    if ((await uploadButtons.count()) > 0) {
      ...
      const tagName = await uploadButton.evaluate((el) =>
  ...
  test('should handle measurement tracking if available', async ({ page }) => {
    ...
    const measurementInputs = page.locator(
      '[data-testid="measurement"], input[placeholder*="weight" i], input[placeholder*="measurement" i]'
    );
...
test.describe('Progress Analytics', () => {
  ...
  test('should calculate workout streaks if available', async ({ page }) => {
    ...
    const streakElements = page.locator(
      '[data-testid="streak"], .streak, [data-testid="current-streak"]'
    );
  ...
  test('should show workout frequency analysis if available', async ({
    page,
  }) => {
    const progressPage = new BasePage(page);
    ...
    const frequencyElements = page.locator(
      '[data-testid="frequency"], .frequency, [data-testid="workout-frequency"]'
    );
