    ...
    check: async () => {
      ...
      if (major >= 18) {
        ...
        return {
          ...
          solution:
            'Update to Node.js 18.17.0 or later. Use nvm or download from nodejs.org',
        };
      }
    },
  },
      ...
      try {
        ...
        if (stdout.trim()) {
          return {
            ...
            solution:
              'Stop the process using port 3000 or use PORT=3001 pnpm dev',
          };
        } else {
        ...
        return {
          status: 'pass',
          message: 'Port check skipped (lsof not available)',
        };
      }
    },
  },
...
async function runTroubleshooting(): Promise<void> {
  log(
    `${colors.blue}${colors.bold}🔧 AI Personal Trainer - Troubleshooting${colors.reset}`
  );
  log('==========================================');
  log('');
    ...
    try {
      ...
      switch (result.status) {
        ...
        case 'warn':
          ...
          if (result.solution) {
            log(
              `  ${colors.cyan}💡 Solution: ${result.solution}${colors.reset}`
            );
          }
          warnCount++;
          break;
  ...
  if (failCount === 0) {
    if (warnCount === 0) {
      ...
      log(
        `✅ Core functionality should work, but some warnings need attention.`,
        colors.yellow
      );
    ...
    log(
      `❌ ${failCount} critical issues found. Please fix them before continuing.`,
      colors.red
    );
    log(`${colors.bold}Critical fixes needed:${colors.reset}`);
    log(`1. Address all failed checks above`);
    log(
      `2. Run troubleshooting again: ${colors.cyan}pnpm troubleshoot${colors.reset}`
    );
  }
