...
async function testWorkoutQueries() {
  try {
    ...
    exercises.forEach((exercise) => {
      console.log(
        `  ✓ ${exercise.name} (${exercise.exercise_type}, ${exercise.difficulty_level})`
      );
      console.log(
        `    Primary muscles: ${exercise.primary_muscle_groups.join(', ')}`
      );
    });
    ...
    `;
    console.log(
      `  ✓ Found ${chestExercises.length} chest exercises using muscle group index`
    );
    ...
    `;
    console.log(
      `  ✓ Found ${strengthExercises.length} strength exercises using type index`
    );
    ...
    try {
      ...
      `;
      console.log(
        '  ❌ Empty name constraint test failed - should have been rejected'
      );
    } catch (error) {
    ...
    try {
      ...
      `;
      console.log(
        '  ❌ Empty muscle groups constraint test failed - should have been rejected'
      );
    } catch (error) {
    ...
    const slugTest =
      await db`SELECT generate_exercise_slug('Test Exercise Name!!! @#$') as slug`;
    console.log(`  ✅ Slug generation working: "${slugTest[0].slug}"`);

    const equipSlugTest =
      await db`SELECT generate_equipment_slug('Fancy Equipment Name!!! @#$') as slug`;
    console.log(
      `  ✅ Equipment slug generation working: "${equipSlugTest[0].slug}"`
    );
