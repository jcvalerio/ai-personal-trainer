  ...
  try {
    ...
    if (
      !RATE_LIMITS.PROFILE_UPDATE.isAllowed(
        `generation_jobs:${userId}:${clientIp}`
      )
    ) {
      await logAuthEvent(
        'rate_limit_exceeded',
        'security',
        'Generation jobs API rate limit exceeded',
        userId
      );
      return NextResponse.json(
    ...
    await logAuthEvent(
      'generation_jobs_access_failed',
      'security',
      'Generation jobs access failed',
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    try {
      ...
      await logAuthEvent(
        'workout_generation_validation_failed',
        'security',
        'Invalid workout generation data',
        userId,
        orgId || undefined,
    ...
    const result = await aiWorkoutService.createWorkoutGenerationJob(
      validatedData,
      context
    );
   ...
   */
  function checkAIFeatureAccess(subscriptionTier: string): {
    allowed: boolean;
    reason?: string;
  } {
