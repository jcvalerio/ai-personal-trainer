  ...
  try {
    ...
    if (
      !RATE_LIMITS.PROFILE_UPDATE.isAllowed(
        `workout_plan_get:${userId}:${clientIp}`
      )
    ) {
      await logAuthEvent(
        'rate_limit_exceeded',
        'security',
        'Workout plan get API rate limit exceeded',
        userId
      );
      return NextResponse.json(
    ...
    await logAuthEvent(
      'workout_plan_access_failed',
      'security',
      'Workout plan access failed',
...
export async function PUT(
  ...
  try {
    ...
    if (
      !RATE_LIMITS.PROFILE_UPDATE.isAllowed(
        `workout_plan_update:${userId}:${clientIp}`
      )
    ) {
      await logAuthEvent(
        'rate_limit_exceeded',
        'security',
        'Workout plan update rate limit exceeded',
        userId
      );
      return NextResponse.json(
        { success: false, error: 'Too many requests', code: 'RATE_LIMITED' },
    ...
    try {
      ...
      await logAuthEvent(
        'workout_plan_update_validation_failed',
        'security',
        'Invalid workout plan update data',
        userId,
        orgId || undefined,
        {
          planId: params.planId,
...
export async function DELETE(
  ...
  try {
    ...
    if (
      !RATE_LIMITS.PROFILE_UPDATE.isAllowed(
        `workout_plan_delete:${userId}:${clientIp}`
      )
    ) {
      await logAuthEvent(
        'rate_limit_exceeded',
        'security',
        'Workout plan delete rate limit exceeded',
        userId
      );
      return NextResponse.json(
        { success: false, error: 'Too many requests', code: 'RATE_LIMITED' },
    ...
    // Delete workout plan
    const result = await workoutService.deleteWorkoutPlan(
      params.planId,
      context
    );
