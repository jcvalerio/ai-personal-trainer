  ...
  try {
    ...
    if (
      !RATE_LIMITS.PROFILE_UPDATE.isAllowed(
        `workout_plans:${userId}:${clientIp}`
      )
    ) {
      await logAuthEvent(
        'rate_limit_exceeded',
        'security',
        'Workout plans API rate limit exceeded',
        userId
      );
      return NextResponse.json(
    ...
    await logAuthEvent(
      'workout_plans_access_failed',
      'security',
      'Workout plans access failed',
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (
      !RATE_LIMITS.PROFILE_UPDATE.isAllowed(
        `create_workout_plan:${userId}:${clientIp}`
      )
    ) {
      await logAuthEvent(
        'rate_limit_exceeded',
        'security',
        'Workout plan creation rate limit exceeded',
        userId
      );
      return NextResponse.json(
        { success: false, error: 'Too many requests', code: 'RATE_LIMITED' },
    ...
    try {
      ...
      await logAuthEvent(
        'workout_plan_validation_failed',
        'security',
        'Invalid workout plan data',
        userId,
        orgId || undefined,
    ...
    const authResult = await auth();
    await logAuthEvent(
      'workout_plan_creation_failed',
      'security',
      'Workout plan creation failed',
      authResult.userId || undefined,
      authResult.orgId || undefined,
      { error: (error as Error).message }
    );
