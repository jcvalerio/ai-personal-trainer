  ...
  try {
    ...
    if (!RATE_LIMITS.PROFILE_UPDATE.isAllowed(`${userId}:${clientIp}`)) {
      await logAuthEvent(
        'rate_limit_exceeded',
        'security',
        'Profile API rate limit exceeded',
        userId
      );
      return NextResponse.json(
  ...
  try {
    ...
    if (existingProfile) {
      await logAuthEvent(
        ...
        'profile',
        'User attempted to create existing profile',
        userId
      );
  ...
  try {
    ...
    if (!RATE_LIMITS.PROFILE_UPDATE.isAllowed(`${userId}:${clientIp}`)) {
      await logAuthEvent(
        'rate_limit_exceeded',
        'security',
        'Profile update rate limit exceeded',
        userId
      );
      return NextResponse.json(
        { success: false, error: 'Too many requests', code: 'RATE_LIMITED' },
        ...
        await client.users.updateUserMetadata(userId, {
          publicMetadata: {
            ...
            ...(updateData.fitnessLevel && {
              fitnessLevel: updateData.fitnessLevel,
            }),
  ...
  try {
    ...
    if (!RATE_LIMITS.AUTH.isAllowed(`delete:${userId}:${clientIp}`)) {
      await logAuthEvent(
        'rate_limit_exceeded',
        'security',
        'Profile deletion rate limit exceeded',
        userId
      );
      return NextResponse.json(
        { success: false, error: 'Too many requests', code: 'RATE_LIMITED' },
