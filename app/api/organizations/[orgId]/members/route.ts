...
export async function GET(
  ...
  try {
    ...
    const clientIp =
      ...
      'unknown';
    if (
      !RATE_LIMITS.PROFILE_UPDATE.isAllowed(`members:${userId}:${clientIp}`)
    ) {
      await logAuthEvent(
        'rate_limit_exceeded',
        'security',
        'Members API rate limit exceeded',
        userId
      );
      return NextResponse.json(
    ...
    const userIsMember = members.some(
      (member) => member.clerkUserId === userId
    );
    if (!userIsMember) {
      await logAuthEvent(
        'unauthorized_access',
        'security',
        'Attempted access to organization members',
        userId,
        orgId
      );
      return NextResponse.json(
    ...
    const userRole =
      members.find((member) => member.clerkUserId === userId)?.role || 'user';
    const isAdmin = ['gym_admin', 'gym_owner', 'family_admin'].includes(
      userRole
    );
...
export async function POST(
  ...
  try {
    ...
    const existingMember = members.find(
      (member) => member.email.toLowerCase() === inviteData.email
    );
    if (existingMember) {
      return NextResponse.json(
        {
    ...
    // Create invitation
    const invitation = await createOrganizationInvite(
      orgId,
      userProfile.id,
      inviteData
    );
...
export async function DELETE(
  ...
  try {
    ...
    const isCurrentUserAdmin = [
      ...
      'gym_owner',
      'family_admin',
    ].includes(currentUserMember.role);
    ...
    if (
      !isSelfRemoval &&
      isTargetOwner &&
      currentUserMember.role !== 'gym_owner'
    ) {
      return NextResponse.json(
        {
          success: false,
    ...
    if (!RATE_LIMITS.PROFILE_UPDATE.isAllowed(`remove:${userId}:${clientIp}`)) {
      await logAuthEvent(
        'rate_limit_exceeded',
        'security',
        'Member removal rate limit exceeded',
        userId
      );
      return NextResponse.json(
        { success: false, error: 'Too many requests', code: 'RATE_LIMITED' },
