  ...
  try {
    ...
    if (!RATE_LIMITS.PROFILE_UPDATE.isAllowed(`orgs:${userId}:${clientIp}`)) {
      await logAuthEvent(
        'rate_limit_exceeded',
        'security',
        'Organizations API rate limit exceeded',
        userId
      );
      return NextResponse.json(
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const clientIp =
      ...
      'unknown';
    if (
      !RATE_LIMITS.PROFILE_UPDATE.isAllowed(`create_org:${userId}:${clientIp}`)
    ) {
      await logAuthEvent(
        'rate_limit_exceeded',
        'security',
    ...
    if (!validationResult.success) {
      await logAuthEvent(
        'organization_creation_validation_failed',
        'organization',
    ...
    if (!orgData.maxMembers) {
      ...
      const limits =
        ORGANIZATION_LIMITS[
          orgData.type.toUpperCase() as keyof typeof ORGANIZATION_LIMITS
        ];
      orgData.maxMembers =
        limits[
          subscriptionTier.toUpperCase() as keyof typeof limits
        ].maxMembers;
    }
    ...
    try {
      ...
      console.log(
        `User ${userId} created organization ${organization.id} with role ${newRole}`
      );
...
export async function PUT(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const clientIp =
      ...
      'unknown';
    if (
      !RATE_LIMITS.PROFILE_UPDATE.isAllowed(`update_org:${userId}:${clientIp}`)
    ) {
      await logAuthEvent(
        'rate_limit_exceeded',
        'security',
        'Organization update rate limit exceeded',
        userId
      );
      return NextResponse.json(
        { success: false, error: 'Too many requests', code: 'RATE_LIMITED' },
    ...
    if (!validationResult.success) {
      await logAuthEvent(
        ...
        'organization',
        'Invalid organization update data',
        userId,
        orgId,
        { errors: validationResult.error.issues }
      );
