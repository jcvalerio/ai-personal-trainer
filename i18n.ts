...
export default getRequestConfig(async ({ locale }) => {
  ...
  if (validLocale !== locale) {
    console.log(
      `i18n.ts - Invalid locale "${locale}", using default:`,
      validLocale
    );
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `Empty or invalid messages object for locale: ${validLocale}`
      );
    }

    console.log(
      'i18n.ts - Successfully loaded messages for:',
      validLocale,
      'Keys:',
    ...
    return {
      ...
      getMessageFallback: ({ namespace, key }) => {
        console.warn(
          `Missing message: ${namespace ? namespace + '.' : ''}${key}`
        );
    ...
    console.error(
      'i18n.ts - Failed to load messages for locale:',
      validLocale,
      error
    );
